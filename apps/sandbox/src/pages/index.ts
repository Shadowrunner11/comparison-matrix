...
const removeControlsIfSlidesMaxOut = ()=>{
  const slidesPerView = getSlidesPerView()
  if(typeof slidesPerView === 'string' || bodySwiper.slides.length <= slidesPerView){
    fixedColumn.classList.remove('table__locked-column--translate')
    return controls.classList.add('hide');
  }

  fixedColumn.classList.add('table__locked-column--translate')
  controls.classList.remove('hide');
}

removeControlsIfSlidesMaxOut()

matchMedia("(min-width: 600px)").addEventListener('change', removeControlsIfSlidesMaxOut);
